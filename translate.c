...
static void ClearBuffer() { ClearList(stmtbuffer); }
...
static Addr TranSlateExp(struct node *Node) {
  ...
  if (strcmp(str, "ID") == 0) {
    ...
    if ((temptype->kind == ARRAY || temptype->kind == STRUCTURE) &&
        IsPtr(tempidname) == 0) {
  ...
  if (strcmp(str, "ExpRELOPExp") == 0) {
    addr1 = ExpUesStar(SonTree[0]->addr);
    addr2 = ExpUesStar(SonTree[2]->addr);
    sprintf(tempstr, "%s %s %s", addr1,
            Node->lson_node->rbro_node->lexical_value.stringvalue, addr2);
    return CreateAddr(tempstr, standtypeint);
  }
  ...
  if (strcmp(str, "ExpDOTID") == 0) {
    ...
    GenStmt(&tempstr);
    sprintf(tempstr, "*%s", addr);
    return CreateAddr(tempstr,
                      FindStructFieldType(SonTree[0]->type, tempidname));
  }
...
static Addr TranSlateVarDec(struct node *Node, int isparam) {
  ...
  if (strcmp(str, "ID") == 0) {
    ...
    if (isparam == 1) {
      ...
      if (ReturnTypeKind(temptype) == ARRAY ||
          ReturnTypeKind(temptype) == STRUCTURE) {
        int typesize;
        typesize = ComputeTypeSize(temptype);
