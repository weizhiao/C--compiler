...
Type ReturnFunDecReturnType(Type fundec) { return fundec->fundec.ReturnType; }

int ReturnTypeKind(Type type) { return type->kind; }
...
void SetArrayType(Type array, Type type) { array->array.arraytype = type; }
...
int IsTypeEqual(Type type1, Type type2) {
  if (type1->kind == type2->kind) {
    ...
    if (type1->kind == FUNDEC) {
      if (IsTypeEqual(type1->fundec.ReturnType, type2->fundec.ReturnType) ==
          0) {
        return 0;
      }
