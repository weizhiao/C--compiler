...
extern int iserror;
extern Type standtypeint;
extern Type standtypefloat;
extern HashNode structhashtable[HASHTABLESIZE];
...
static Type Top() { return specifierstack->specifier; }

static Type UnderTop() { return specifierstack->next->specifier; }
...
static Type AnalysisExp(struct node *Node) {
  ...
  if (strcmp(str, "ExpANDExp") == 0 || strcmp(str, "ExpORExp") == 0) {
    if (IsTypeEqual(SonTree[0], standtypeint) == 0 ||
        IsTypeEqual(SonTree[2], standtypeint) == 0) {
      PrintError(7, Node->line, "Logical operations can only use int");
    }
    return CreateBasic("int", NOLEFTVALUE);
  }
  if (strcmp(str, "ExpPLUSExp") == 0 || strcmp(str, "ExpMINUSExp") == 0 ||
      strcmp(str, "ExpSTARExp") == 0 || strcmp(str, "ExpDIVExp") == 0) {
    if (ReturnTypeKind(SonTree[0]) == BASIC &&
        ReturnTypeKind(SonTree[2]) == BASIC) {
      if (IsTypeEqual(SonTree[0], SonTree[2]) == 0) {
        PrintError(7, Node->line, "Arithmetic operation type mismatch");
      }
      return SonTree[0];
    } else {
      PrintError(7, Node->line,
                 "Only int and float can participate in arithmetic operations");
      return CreateBasic("int", NOLEFTVALUE);
    }
  }
  ...
  if (strcmp(str, "ExpDOTID") == 0) {
    if (ReturnTypeKind(SonTree[0]) != STRUCTURE) {
      ...
      char *fieldname =
          Node->lson_node->rbro_node->rbro_node->lexical_value.stringvalue;
  ...
  if (strcmp(str, "IDLPArgsRP") == 0) {
    ...
    // 此时只有一种可能
    assert(strcmp(str, "ExpLBExpRB") == 0);
